...
Sequential, start to finish: setup → first login → users → connectors → projects → workbenches → sales. See [the guides](/docs/guides/README.md).

## Architecture — how XANA is built

- [Architecture overview](/docs/ARCHITECTURE.md) — the technical deep-dive: how the investigation engine reasons about a case, how the underlying AI model is configured, the retrieval pipeline behind every citation, and how the interface and hub fit around it.
...
- [Running it locally](/docs/setup/docker-deployment.md) — the local/single-host path.
- [Running it in production](/docs/setup/kubernetes-deployment.md) — how a real deployment is structured and kept up to date.
