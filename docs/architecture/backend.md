...
| `developer` | A hidden, unguarded debug-only surface (`GET /developer/rag-search-log`) — intentionally not linked from any nav; backs `frontend/app/developer`. |
...
## Auth coverage — honest state

There is no global auth guard — `app.module.ts` only registers a request-logging middleware for all routes. Route protection is a per-controller, manual check against `AuthService`, applied inconsistently: as of this writing, **9 of 18 controllers** reference `AuthService` at all (`auth`, `connectors`, `mcp-servers`, `projects`, `sales-report-config`, `sales-report-trigger`, `users`, `workbenches`, `workflow-agent-status`). The rest — including `ai-providers`, `knowledge`, `ontology`, `proxy`, and `support` — have no auth check in the controller at all (`app` and `health` being open is expected/fine for those two). Don't assume a route is protected without checking its controller directly. This is also why the "Keycloak / IFRIC Registry" item in the root README's pending list matters beyond just replacing the login screen — today's auth isn't just a different implementation, it's inconsistently applied.
